<template>
  <v-card class="entry-card" variant="outlined">
    <v-card-text>
      <p class="entry-field"><strong>Service: </strong>{{ entry.service }}</p>
      <p class="entry-field"><strong>Email: </strong>{{ entry.email }}</p>
      <p class="entry-field"><strong>Username: </strong>{{ entry.username }}</p>
      <p v-show="showSensitive" class="entry-field sensitive">
        <strong>Password: </strong>{{ entry.password }}
      </p>
    </v-card-text>
    <v-card-actions>
      <v-btn size="small" variant="text" color="primary" @click="copyValue(entry.password)">
    ...
    <v-dialog v-model="confirmDelete" max-width="350">
      <v-card>
        ...
        <v-card-text>This will permanently delete this password entry.</v-card-text>
        <v-card-actions>
          <v-spacer />
