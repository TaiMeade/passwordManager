  ...
  ipcMain.handle('db:search', (_, term) => {
    ...
    return results.map(row => {
      ...
      switch (row.entry_type) {
        case 'password': decrypted.password = decrypt(row.password); break
        case 'card': decrypted.cvv = decrypt(row.cvv); break
        case 'bank': decrypted.account = decrypt(row.account); break
        case 'id': decrypted.id_number = decrypt(row.id_number); break
        case 'note': decrypted.content = decrypt(row.content); break
      }
      return decrypted
    })
  })
